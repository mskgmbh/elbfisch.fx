 ...
 */
public class CharStringOutputFieldTestModule extends Module{
    protected CharString    charStringSignal;
    protected PeriodOfTime  pot;
    
    public CharStringOutputFieldTestModule(AbstractModule containingModule, String identifier){
    ...
    protected void work() throws ProcessException {
        try{
            ...
            int i = 0;
            do{
                if(!charStringSignal.isValid()){
                    pot.await();
                    pot.await();
                    charStringSignal.set("abcd");
                }
                charStringSignal.set(charStringSignal.get() + "X");
                Log.info("char string signal changed " + charStringSignal.get());
                pot.await();
                if (i++ % 10 == 1){
                    charStringSignal.invalidate();
                    pot.await();
                    pot.await();
                    charStringSignal.set("abcd");
                }
